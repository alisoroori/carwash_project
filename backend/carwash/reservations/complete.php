...
error_log('carwash/reservations/complete.php: request from ' . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
...
try {
    ...
    error_log('carwash/reservations/complete.php: booking ' . $bookingId . ' set to ' . $newStatus . ' by carwash ' . $carwashId);

    Response::success('OK', ['booking_id' => $bookingId, 'status' => $newStatus]);

} catch (Throwable $e) {
    error_log('carwash/reservations/complete.php ERROR: ' . $e->getMessage());
