...
// Prevent PHP notices/warnings from being emitted into JSON
...
error_log('carwash/reservations/approve.php: request from ' . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
...
try {
    ...
    $stmt = $pdo->prepare('SELECT id, status FROM bookings WHERE id = :id AND carwash_id = :cw LIMIT 1');
    ...
    error_log('carwash/reservations/approve.php: booking ' . $bookingId . ' set to ' . $newStatus . ' by carwash ' . $carwashId);

    Response::success('OK', ['booking_id' => $bookingId, 'status' => $newStatus]);

} catch (Throwable $e) {
    error_log('carwash/reservations/approve.php ERROR: ' . $e->getMessage());
