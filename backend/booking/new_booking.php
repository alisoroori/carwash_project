      ...
      <select id="carwashSelect" name="carwash_id" class="w-full px-4 py-2 border rounded">
        ...
        <?php foreach ($carwashes as $cw): ?>
          <option value="<?php echo (int)$cw['id']; ?>" data-city="<?php echo htmlspecialchars($cw['city'] ?? ''); ?>" data-district="<?php echo htmlspecialchars($cw['district'] ?? ''); ?>" <?php echo ($selectedCarwashId && $selectedCarwashId == $cw['id']) ? 'selected' : ''; ?>>
            <?php echo htmlspecialchars($cw['name'] . ' — ' . ($cw['district'] ?? '') . ' / ' . ($cw['city'] ?? '')); ?>
          </option>
        <?php endforeach; ?>
      </select>
    </div>
    ...
    <div>
      ...
      <select id="serviceSelect" name="service_id" class="w-full px-4 py-2 border rounded">
        ...
        <?php foreach ($services as $s): ?>
          <option value="<?php echo (int)$s['id']; ?>" data-price="<?php echo htmlspecialchars($s['price'] ?? ''); ?>"><?php echo htmlspecialchars($s['name'] . ' — ' . ($s['price'] ?? '')); ?></option>
        <?php endforeach; ?>
      </select>
    ...
    (function(){
          ...
          if (opt) {
            ...
            if (el('districtSelect')) {
              ...
              if (window.districtsByCity && window.districtsByCity[cityVal]) {
                el('districtSelect').innerHTML = '<option value="">-- Mahalle Seçiniz --</option>' + window.districtsByCity[cityVal].map(d=>`<option value="${d}" ${d===district?'selected':''}>${d}</option>`).join('');
              } else if (district) {
                el('districtSelect').innerHTML = '<option value="">-- Mahalle Seçiniz --</option>' + `<option value="${district}" selected>${district}</option>`;
              }
            }

          }
          // load services
          if (el('serviceSelect')) el('serviceSelect').innerHTML = '<option value="">-- Hizmet Seçiniz --</option>';
          if (!val) return;
          try {
            ...
            if (Array.isArray(json)) {
              json.forEach(s => {
                ...
                o.textContent = (s.name || ('Hizmet ' + (s.id||''))) + (s.price?(' — '+s.price):'');
      ...
      if (el('citySelect')) {
        el('citySelect').addEventListener('change', function(){
          ...
          el('districtSelect').innerHTML = '<option value="">-- Mahalle Seçiniz --</option>';
          if (!city) return;
