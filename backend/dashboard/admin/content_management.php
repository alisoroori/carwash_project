    ...
    <div id="pageModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
        <div class="relative top-20 mx-auto p-5 border w-3/4 shadow-lg rounded-md bg-white">
            ...
            <form id="pageForm" class="space-y-4">
                <label for="pageId" class="sr-only">Id</label><input type="hidden" name="id" id="pageId">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700" for="pageTitle">Başlık</label>
                        <label for="pageTitle" class="sr-only">Title</label><input type="text" name="title" id="pageTitle" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700" for="pageSlug">URL</label>
                        <label for="pageSlug" class="sr-only">Slug</label><input type="text" name="slug" id="pageSlug" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700" for="pageContent">İçerik</label>
                    <label for="pageContent" class="sr-only">Content</label><textarea name="content" id="pageContent" rows="10" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"></textarea>
                </div>
                <div class="flex justify-end space-x-2">
                    <button type="button" onclick="closeModal('pageModal')" class="px-4 py-2 text-gray-600 hover:text-gray-800">
                        İptal
                    </button>
